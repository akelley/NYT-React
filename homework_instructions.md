...
1. [Check out the example solution](https://react-is-awesome.herokuapp.com/) this should give you an ida for how the app should function.
...
5. Create a Node/Express/MongoDB/ReactJS app called `nytreact`. This will be a recreation of the [NYT Articles Search](http://nytarticle-search.herokuapp.com/) exercise application we built back in [Week 6](../../../06-ajax/01-Activities/16-NYTSearch/Solved/NYTArticleSearch_Best_Solution/nyt-example.html). Running this application will:
   ...
   * Create a Bootstrap layout similar to that displayed in [the example solution](https://react-is-awesome.herokuapp.com/). This should be a SPA (Single Page Application) that uses [`react-router`](https://github.com/reactjs/react-router) to navigate, hide and show your React components without changing the route within Express.
   ...
   * At a minimum, your application should include 3 components, `App`, `Home`, and `Saved`.

     * **App** - Holds all other components, including React Routes.

     * **Home** - Renders the home page and displays articles retrieved from the New York Times.

     * **Saved** - Renders the saved articles page and displays articles saved by the user to the mongodb.

   * Though it is encouraged to experiement with further breaking up your application's components into smaller pieces once you have the basic functionality working.
